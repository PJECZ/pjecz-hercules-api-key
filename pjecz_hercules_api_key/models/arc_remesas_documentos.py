...
class ArcRemesaDocumento(Base, UniversalMixin):
    ...
    # Columnas
    anomalia: Mapped[Optional[str]] = mapped_column(
        Enum(*ANOMALIAS, name="arc_remesas_documentos_anomalias", native_enum=False), index=True
    )
    fojas: Mapped[int]
    observaciones_solicitante: Mapped[Optional[str]] = mapped_column(String(256))
    observaciones_archivista: Mapped[Optional[str]] = mapped_column(String(256))
